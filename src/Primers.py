...
def make_primers_dir(based_path):
	"""
	Creates a new folder into an specific path

	:param based_path: the basic path where this action will take place. It's an String.

	:returns: the full new path
	"""
...
def generate_basic_primers_input(filename,seq_id,seq,seq_target,product_size=DEFAULT_PRODUCT_SIZE):
	"""
	Generates a basic primer3_core's input settings file that will be used for getting primers.

	:param filename: the primers input filename as String.

	:param seq_id: An String that specifies the file full path and filename. Ex.: "/home/usr/documents/prueba/prueba"_"E7"_"th7"_"primers"_""

	:param seq: specifies the sequence data. It's an String. 

	:param seq_target: An String with the format number,number. Ex.: "37,21"

	:param product_size: An String with the format number-number. Ex.: "100-300"
	"""
...
def add_parameter_to(primers_input_filename,prefix,value,gui_obj):
	"""
	Addes an specific parameter to the basic primer3_core's input settings file.
	
	:param primers_input_filename: the basic primer3_core's input settings filename as String.

	:param prefix: specific constant string that describes an input.

	:param value: an String. The value of an spacific parameter at the input settings.

	:param gui_obj: gui object
	"""
	try:
...
def get_primers(input_filename,gui_obj):
	"""
	Executes the primer3_core program using the primer3_core's input file.

	:param input_filename: an String that tell us the full filename.

	:param gui_obj: gui object

	:returns: primer3_core results
	"""
	try:
		gui_obj.println("\nCalculing primers")
...
def read_primers_file(filename,gui_obj):
	"""
	Reads an specific primers file that was generated by primer3_core.

	:param filename: the primers filename as String.

	:param gui_obj: gui object

	:returns: an specific primers data
	"""
	try:
...
def get_file_data(filename,file_type,gui_obj):
	"""
	Reads the files that was generated by primer3_core.

	:param filename: the primers filename as String.

	:param file_type: the primers file extention. Ex.: ".for", ".rev",etc.

	:param gui_obj: gui object

	:returns: full primers data
	"""
	try:
...
def set_primers_data_format(data):
	"""
	Sets the format to the obtened primers data .

	:param data: the primers data

	:returns: an spacific primers data with an specific format (title+data)
	"""
...
def read_primers(based_filename,gui_obj):
	"""
	Reads the genereted primers files.

	:param based_filename: the primers based filename (as String).

	:param gui_obj: gui object

	:returns: full and formated primers data.
	"""
...
def get_primers_list(filename,gui_obj):
	"""
	Reads the files that was generated by primer3_core and creates a primer list

	:param filename: the primers filename as String.

	:param gui_obj: gui object

	:returns: primers list. It's an String list
	"""
	text = ''
...
def split_blast_results(results):
	...
	:param results: the full Blast executation results. (as String)

	:returns: blast filtered (splitted) results. It's an String.
	"""
...
def run_blast(gui_obj,sequence,blast_program=BLAST_PROGRAM ,database=BLAST_DATABASE):#,output_filename): solo si format_type != "Text"
	...
	:param blast_program: (Default "blastn")

	:param database: (Default "nt")
...
def get_blast_data(primers_list,gui_obj,blast_program=BLAST_PROGRAM ,database=BLAST_DATABASE):
	"""
	Gets the blast data obteined for the 3 first primer sequence at primers list.

	:param gui_obj: gui object

	:param primers_list: list obteined by get_primers_list(filename,gui_obj) function. It's an String list.
